...
type UpgradeRow = { store_id: string; created_at: string; contact_phone: string | null; note: string | null }
...
export default async function AdminStoresPage() {
  ...
  const [storesRes, statsRes, usersRes, credsRes, upgradesRes] = await Promise.all([
    supabase
      .from('stores')
      .select(
    ...
    supabase
      ...
      .select('store_id, created_at, contact_phone, note')
      .eq('status', 'pending'),
  ])
  ...
  const upgradeByStore = new Map<string, UpgradeRow>()
  for (const u of (upgradesRes.data || []) as UpgradeRow[]) upgradeByStore.set(u.store_id, u)
  ...
  const stores: AdminStore[] = rows.map((store) => {
    ...
    return {
      ...
      upgradeRequest: upgrade
        ? { createdAt: upgrade.created_at, phone: upgrade.contact_phone, note: upgrade.note }
        : null,
      billing: {
